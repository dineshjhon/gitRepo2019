	...
	stages {
		stage('Set Environment') {
			steps {
				...
				script {
					...
					branch = "${RepoName}"+"${build_version}"
					git_url_id = "$RepoName"+"_git_url"
					...
					git_User = props["git_User"]
				
					branch_source_path = "$jenkins_ws"+"/"+"${RepoName}"+"/source"
					branch_build_path = "$jenkins_ws"+"/"+"${RepoName}"+"/build"+"/"+"$build_version"
					...
					project_name = "$RepoName"+"_is_"+"_project"
					dep_repo_name= "$RepoName"+"_is_"+"_repo"
					...
					target_alias = props["$TargetEnvironment"+'_alias_is']
					target_host = props["$TargetEnvironment"+'_target_host_is']
					target_port = props["$TargetEnvironment"+'_target_port_is']
					target_user = props["$TargetEnvironment"+'_target_user_is']
					target_password = props["$TargetEnvironment"+'_target_password_is']
					target_version = props["$TargetEnvironment"+'_target_version_is']					
				}
			}
		}
		stage('Checkout Source code') {
			steps {
				...
				gitCheckout("$branch_source_path", "$branch", "git_User","$git_credentials", "$git_url")
			}
		}
...
def gitCheckout(branchDir, branchName, git_User, credentialsId, gitURL) {
	dir("$branchDir") {
		git branch: "$branchName", git_User: "$git_User", credentialsId: "$credentialsId", url: "$gitURL"
